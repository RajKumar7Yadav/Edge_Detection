...
// https://naveenideas.blogspot.com/2018/02/sobel-edge-detection.html
...
// https://en.wikipedia.org/wiki/Canny_edge_detector#:~:text=Edge%20tracking%20by%20hysteresis,-Canny%20edge%20detection&text=Usually%20a%20weak%20edge%20pixel,its%208%2Dconnected%20neighborhood%20pixels.
// For thetha Value calculation:
// http://pages.drexel.edu/~nk752/Research/cannyTut2.html#Step%204
...
Mat Sobelfilter(Mat gblur)
{
	...
	for (int i = 1; i < gblur.rows - 1; i++)
	{
		for (int j = 1; j < gblur.cols - 1; j++)
		{
			...
			G = sqrt((gx * gx) + (gy * gy)); //abs(gx) + abs(gy); abs value is leading to disapp of pixel in lane
			G = G > 255 ? 255 : G;
			G = G < 0 ? 0 : G;
